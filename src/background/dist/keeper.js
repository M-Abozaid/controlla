...
var Keeper = /** @class */ (function () {
    function Keeper() {
        var _this = this;
        this.controlIsPaused = false;
        this.controlTab = function (tab, shouldIncrementQuota) {
            if (shouldIncrementQuota === void 0) { shouldIncrementQuota = true; }
            return __awaiter(_this, void 0, Promise, function () {
                var rules, effectiveRules, matchingRules, visit, highestPriority, _a, tabExpired, visibilityExpired;
                var _this = this;
                    ...
                    switch (_b.label) {
                        ...
                        case 1:
                            ...
                            if (!effectiveRules.length)
                                return [2 /*return*/];
                            matchingRules = [];
                            visit = storage_1["default"].getOpenVisit(tab.id);
                            if (!visit) {
                            ...
                            if (!this.isVisitActive(visit))
                                return [2 /*return*/];
                            matchingRules = effectiveRules.filter(function (rule) { return rule.doesMatch(tab, visit); });
                            highestPriority = Math.max.apply(Math, matchingRules.map(function (r) { return r.ruleObj.priority || 1; }));
                            matchingRules = matchingRules.filter(function (rule) { return (rule.ruleObj.priority || 1) === highestPriority; });
                        ...
                        case 4:
                            ...
                            if ((tabExpired || visibilityExpired) && storage_1["default"].isControlPaused()) {
                                if (shouldIncrementQuota) {
                                    storage_1["default"].incrementPauseUsage(settings_1["default"].tickDuration);
                                }
                                return [2 /*return*/];
                            }
                            if (tabExpired) {
        ...
        this.run = function () {
            try {
                chrome_promise_1["default"].tabs
                    .query({ active: true })
                    .then(function (activeTabs) { return __awaiter(_this, void 0, void 0, function () {
                    ...
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                ...
                                _a.label = 1;
                            case 1:
                                if (!(_i < activeTabs_1.length)) return [3 /*break*/, 7];
                                tab = activeTabs_1[_i];
                                _a.label = 2;
                            case 2:
                                _a.trys.push([2, 5, , 6]);
                                if (!(tab.url.indexOf('http') === 0)) return [3 /*break*/, 4];
    ...
    Keeper.prototype.incrementQuota = function (rules) {
        return Promise.all(rules.map(function (rule) {
            return storage_1["default"].incrementOrAddUsage(rule._id, settings_1["default"].tickDuration);
        }));
    };
    ...
    Keeper.prototype.isYTVideoAllowed = function (video) {
        return __awaiter(this, void 0, Promise, function () {
            var YTRules, matchingRules, highestPriority, disallowed;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                    ...
                    case 1:
                        ...
                        matchingRules = YTRules.filter(function (rule) {
                            return rule.isEffectiveNow() &&
                                rule.ruleObj.matchers.some(function (matcher) {
                                    return ruleMatcher_1["default"].matchVideoSnippet(matcher, video);
                                });
                        });
                        highestPriority = Math.max.apply(Math, matchingRules.map(function (r) { return r.ruleObj.priority || 1; }));
                        matchingRules = matchingRules.filter(function (rule) { return (rule.ruleObj.priority || 1) === highestPriority; });
    ...
    Keeper.prototype.isVisitActive = function (visit) {
        var _a;
        var activeThresholdAgo = new Date(Date.now() - settings_1["default"].visitActiveThreshold);
        // check if audio is playing
        if (((_a = visit.audibleState) === null || _a === void 0 ? void 0 : _a.length) &&
    ...
    Keeper.prototype.pauseControlUsage = function () {

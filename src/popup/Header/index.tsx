...
const Header = () => {
  ...
  const [isControlPaused, setIsControlPaused] = useState(
    storage.isControlPaused()
  )
        ...
        <div className={`header__main black`}>
          ...
          <div>
            <div className='header__button'>
              ...
              >
                <Add
                  onClick={() => chrome.tabs.create({ url: '/add-rule.html' })}
                />
              </Tooltip>
            </div>
            <button
              className='header__button'
              onClick={toggleIsControlPaused}
              disabled={!isControlPauseAllowed}
            >
              <Tooltip
                TransitionComponent={Zoom}
                TransitionProps={{ timeout: 300 }}
